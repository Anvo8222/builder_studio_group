...
function FilterByCategory({ handleChangeProduct }) {
  const categories = useSelector((state) => state.category.items);

  const handleChangeFilterProduct = (e) => {
    handleChangeProduct(e.target.value, e.currentTarget.checked);
  };
  return (
    <>
      ...
      <ul>
        {categories?.map((category, index) => (
          ...
          >
            <label className="relative pl-[30px] text-[#676b7e] font-light text-sm flex items-center leading-[50px] block w-[100%] cursor-pointer">
              {category.name}
              <input
